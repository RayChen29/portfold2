...
//   let billRange = 
  ...
  for(let i = 0; i < users.length; i++){
    if(users[i].customerId === subscription.customerId){
        custCount++;
    }
  }
let fullBill = 0
  for(let i = 0; i < users.length; i++){
    if(users[i].customerId === subscription.customerId){
        ...
        let billDate = firstDayOfMonth(new Date(parseInt(year),parseInt(month)));
        let finalDate = users[i].deactivatedOn;
        if(finalDate == null){
            finalDate = billDate;
        }
        let firstDate = users[i].activatedOn;

        // let totalCharge = 0
        let days = 0
        while(firstDate >= finalDate){
            days++;
            firstDate = nextDay(firstDate);
        }
        fullBill += ((subscription.monthlyPriceInCents * (days/30) * custCount));
    }
  }
  return fullBill;
